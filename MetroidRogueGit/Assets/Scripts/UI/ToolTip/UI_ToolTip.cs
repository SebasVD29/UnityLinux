...
public class UI_ToolTip : MonoBehaviour
{
    ...
    public virtual void ShowToolTip(bool show, Transform targetRect)
    {
        if (show == false)
        {
            rect.position = new Vector2(9999, 9999);
        ...
        UpdatePositionWithPlayer(targetRect.position);
    }
    ...
    void UpdatePositionWithPlayer(Vector3 targetRect)
    {
        // Convierte de mundo → pantalla
        Vector2 targetPosition = Camera.main.WorldToScreenPoint(targetRect);
        offset = new Vector2(600, 40);
        float screenCenterX = Screen.width / 2;
        float screenTop = Screen.height;
        float screenBottom = 0;

        // Decide si ponerlo a la derecha o izquierda
        targetPosition.x = targetPosition.x > screenCenterX ? targetPosition.x - offset.x : targetPosition.x + offset.x;
